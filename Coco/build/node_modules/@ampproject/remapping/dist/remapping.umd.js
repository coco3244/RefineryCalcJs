(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('@jridgewell/trace-mapping'), require('sourcemap-codec')) : typeof define === 'function' && define.amd ? define(['@jridgewell/trace-mapping', 'sourcemap-codec'], factory) : (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.remapping = factory(global.traceMapping, global.sourcemapCodec));
})(this, function (TraceMap, sourcemapCodec) {
  'use strict';
  ...
  class FastStringArray {
    ...
    put(key) {
      const {
        array,
        indexes
      } = this; // The key may or may not be present. If it is present, it's a number.
  ...
  class SourceMapTree {
    ...
    traceMappings() {
      ...
      function trace(genLine, genColumn, sourcesIndex, sourceLine, sourceColumn, namesIndex) {
        ...
        if (sourcesIndex !== -1) {
          ...
          traced = source.originalPositionFor(sourceLine, sourceColumn, namesIndex === -1 ? '' : rootNames[namesIndex]); // If the trace is invalid, then the trace ran into a sourcemap that doesn't contain a
        ...
        const {
          content,
          filename
        } = traced.source; // Store the source location, and ensure we keep sourcesContent up to
        // date with the sources array.

        const sIndex = sources.put(filename);
        sourcesContent[sIndex] = content;

        if (lastGenLine === genLine && lastSourcesIndex === sIndex && lastSourceLine === line && lastSourceColumn === column) {
    ...
    originalPositionFor(line, column, name) {
      const segment = this.map.traceSegment(line, column); // If we couldn't find a segment, then this doesn't exist in the sourcemap.
  ...
  function build(map, loader) {
    ...
    const children = resolvedSources.map((sourceFile, i) => {
      const source = sourceFile || ''; // Use the provided loader callback to retrieve the file's sourcemap.
      // TODO: We should eventually support async loading of sourcemap files.

      const sourceMap = loader(source); // If there is no sourcemap, then it is an unmodified source file.
